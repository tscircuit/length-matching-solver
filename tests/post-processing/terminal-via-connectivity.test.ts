...
test("preserves endpoint connectivity and port metadata for terminal-via traces", () => {
  const makeTrace = (id: string, name: string, y: number): SimplifiedPcbTrace => ({
    ...
    route: [
      { route_type: "via", x: 0, y, from_layer: "top", to_layer: "bottom", via_diameter: 0.4 },
      { route_type: "wire", x: 0, y, width: 0.2, layer: "bottom", start_pcb_port_id: `${id}_start` },
      { route_type: "wire", x: 10, y, width: 0.2, layer: "bottom", end_pcb_port_id: `${id}_end` },
      { route_type: "via", x: 10, y, from_layer: "bottom", to_layer: "top", via_diameter: 0.4 },
    ],
  })
