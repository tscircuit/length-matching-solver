...
test("matches around native fixed copper without counting it toward logical lengths", (): void => {
  const hdRoutes: HighDensityRoute[] = [10, 12].map(
    (length, index): HighDensityRoute => ({
  ...
  const solver = new LengthMatchingSolver({
    hdRoutes,
    traces: [trace],
    originalConnections: hdRoutes.map(
      (route): SimpleRouteConnection => ({
        name: route.connectionName,
        pointsToConnect: route.route.map(
  ...
  expect(
    Math.abs(
      getRouteLength(matchedHdRoutes[0]!) - getRouteLength(matchedHdRoutes[1]!),
    ),
