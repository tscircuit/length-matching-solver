...
export const createSearchGeometryValidator = (input: {
  ...
  const obstacleContains = (point: Point, obstacle: Obstacle, inflation: number): boolean => {
    ...
    return Math.abs(x) <= obstacle.width / 2 + inflation && Math.abs(y) <= obstacle.height / 2 + inflation
  }
  const obstacleIsOnLayer = (obstacle: Obstacle, layer: string): boolean => {
    const z = getLayerIndex(layer, input.layerCount)
    return obstacle.layers.includes(layer) || obstacle.zLayers?.includes(z) === true
  }
  const laneSegments = (start: CoupledPathPoint, end: CoupledPathPoint): CopperSegment[] => {
    const length = Math.hypot(end.x - start.x, end.y - start.y)
    if (length === 0) return []
    const normal = {
      x: (-(end.y - start.y) / length) * input.side * input.centerlineSpacing / 2,
      y: ((end.x - start.x) / length) * input.side * input.centerlineSpacing / 2,
    }
    const isStartTerminal = samePoint(start, input.start)
    const isEndTerminal = samePoint(end, input.end)
    const terminal: CopperSegment["terminal"] = isStartTerminal && isEndTerminal
      ? "both"
      : isStartTerminal
        ? "start"
        : isEndTerminal
  ...
  const segmentIsClear = (segment: CopperSegment): boolean => {
    ...
    for (const point of [segment.start, segment.end]) {
      if (
        point.x - radius < input.bounds.minX || point.x + radius > input.bounds.maxX ||
        point.y - radius < input.bounds.minY || point.y + radius > input.bounds.maxY
      ) return false
    }
    for (const obstacle of input.obstacles) {
      if (!obstacleIsOnLayer(obstacle, segment.layer)) continue
      const [startTerminal, endTerminal] = segment.connectionName === input.firstConnectionName
      ...
      const progressesOutOfStart =
        ...
        Math.hypot(segment.end.x - startTerminal.x, segment.end.y - startTerminal.y) >=
          Math.hypot(segment.start.x - startTerminal.x, segment.start.y - startTerminal.y)
      const progressesIntoEnd =
        obstacleContains(endTerminal, obstacle, 0) &&
        (obstacleContains(segment.start, obstacle, radius + segment.width) ||
          obstacleContains(segment.end, obstacle, radius + segment.width)) &&
        Math.hypot(segment.end.x - endTerminal.x, segment.end.y - endTerminal.y) <=
          Math.hypot(segment.start.x - endTerminal.x, segment.start.y - endTerminal.y)
      ...
      if (obstacleContains(segment.start, obstacle, inflation) || obstacleContains(segment.end, obstacle, inflation)) return false
      const sampleCount = Math.max(2, Math.ceil(Math.hypot(segment.end.x - segment.start.x, segment.end.y - segment.start.y) / 0.2))
      for (let index = 1; index < sampleCount; index++) {
        const t = index / sampleCount
        if (obstacleContains({ x: segment.start.x + (segment.end.x - segment.start.x) * t, y: segment.start.y + (segment.end.y - segment.start.y) * t }, obstacle, inflation)) return false
      }
    }
    for (const other of immutableSegments) {
      if (other.layer !== segment.layer) continue
      const required = segment.width / 2 + other.width / 2 + Math.max(segment.width, other.width)
      if (getMinimumSegmentDistance(segment.start, segment.end, other.start, other.end) < required) return false
    }
    for (const via of immutableVias) {
      if (!via.layers.includes(segment.layer)) continue
      if (pointToSegmentDistance(via, segment.start, segment.end) < segment.width / 2 + via.diameter / 2 + segment.width) return false
    }
    return true
  }
  const viaIsClear = (via: CopperVia): boolean => {
    ...
    if (
      via.x - radius < input.bounds.minX || via.x + radius > input.bounds.maxX ||
      via.y - radius < input.bounds.minY || via.y + radius > input.bounds.maxY
    ) return false
    for (const obstacle of input.obstacles) {
      if (!via.layers.some((layer) => obstacleIsOnLayer(obstacle, layer))) continue
      const exitsConnectedTerminal = via.terminal !== null &&
        obstacle.connectedTo.includes(via.connectionName) && obstacleContains(via, obstacle, 0)
      if (!exitsConnectedTerminal && obstacleContains(via, obstacle, radius + via.diameter)) return false
    }
    for (const segment of immutableSegments) {
      if (!via.layers.includes(segment.layer)) continue
      const required = radius + segment.width / 2 + Math.max(via.diameter, segment.width)
      if (pointToSegmentDistance(via, segment.start, segment.end) < required) return false
    }
    for (const other of immutableVias) {
      if (!other.layers.some((layer) => via.layers.includes(layer))) continue
      const required = radius + other.diameter / 2 + Math.max(via.diameter, other.diameter)
  ...
  return {
    isEdgeValid: (start, end) =>
      start.layer === end.layer && laneSegments(start, end).every(segmentIsClear),
    isViaValid: (point, toLayer, direction) => {
      ...
      const normal = {
        x: (-direction.y / directionLength) * input.side * input.centerlineSpacing / 2,
        y: (direction.x / directionLength) * input.side * input.centerlineSpacing / 2,
      }
      const terminal = samePoint(point, input.start) ? "start" : samePoint(point, input.end) ? "end" : null
