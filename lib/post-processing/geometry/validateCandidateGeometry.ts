...
import type { CopperSegment, CopperVia, ParsedTrace, Point } from "../model/internal-types"
...
const isObstacleOnLayer = (
  ...
  return obstacle.layers.includes(layer) || obstacle.zLayers?.includes(layerIndex) === true
}
...
const isSegmentValid = (
  ...
  for (const obstacle of context.obstacles) {
    if (!isObstacleOnLayer(obstacle, segment.layer, context.layerCount)) continue
    const terminalPoints = segment.terminal === "both"
    ...
    const exitsConnectedTerminalObstacle =
      ...
      terminalPoints.some((terminalPoint) => {
        ...
        return Math.abs(localTerminal.x) <= obstacle.width / 2 + EPSILON &&
          Math.abs(localTerminal.y) <= obstacle.height / 2 + EPSILON
      })
    if (
      !exitsConnectedTerminalObstacle &&
      segmentTouchesInflatedObstacle(segment, obstacle, radius + segment.width)
    )
      return false
  }
  for (const other of immutableCopper.segments) {
    ...
    const requiredDistance =
      segment.width / 2 + other.width / 2 + Math.max(segment.width, other.width)
    if (
      getMinimumSegmentDistance(
        segment.start,
        segment.end,
  ...
  for (const via of immutableCopper.vias) {
    ...
    const requiredDistance = segment.width / 2 + via.diameter / 2 + segment.width
    if (
...
const isViaValid = (
  ...
  if (
    ...
    return false
  for (const obstacle of context.obstacles) {
    if (!via.layers.some((layer) => isObstacleOnLayer(obstacle, layer, context.layerCount)))
      continue
  ...
  for (const segment of immutableCopper.segments) {
    if (!via.layers.includes(segment.layer)) continue
    const requiredDistance = radius + segment.width / 2 + Math.max(via.diameter, segment.width)
    if (pointToSegmentDistance(via, segment.start, segment.end) < requiredDistance - EPSILON)
      return false
  }
  for (const other of immutableCopper.vias) {
    if (!other.layers.some((layer) => via.layers.includes(layer))) continue
    const requiredDistance = radius + other.diameter / 2 + Math.max(via.diameter, other.diameter)
    if (Math.hypot(via.x - other.x, via.y - other.y) < requiredDistance - EPSILON)
      return false
  }
  return true
}

const membersRemainSeparate = (first: ParsedTrace, second: ParsedTrace): boolean => {
  ...
  for (const firstVia of first.vias) {
    for (const secondVia of second.vias) {
      if (!firstVia.layers.some((layer) => secondVia.layers.includes(layer))) continue
      if (
...
const hasSelfCollision = (parsed: ParsedTrace): boolean => {
  for (let firstIndex = 0; firstIndex < parsed.segments.length; firstIndex++) {
  ...
  for (const via of parsed.vias) {
    for (const segment of parsed.segments) {
      ...
      const touchesViaEndpoint =
        Math.hypot(via.x - segment.start.x, via.y - segment.start.y) <= EPSILON ||
