...
export const resolvePostProcessingGridConfig = (input: {
  ...
  const innerGridStep = input.config?.innerGridStep ?? input.defaultInnerGridStep
  const outerGridStep = input.config?.outerGridStep ?? innerGridStep * 4
  const outerPerimeterWidth = input.config?.outerPerimeterWidth ??
  ...
  const innerAxisXCount = Math.ceil((width - outerPerimeterWidth * 2) / innerGridStep) + 2
  const innerAxisYCount = Math.ceil((height - outerPerimeterWidth * 2) / innerGridStep) + 2
  const nodeCountUpperBound = outerAxisXCount * outerAxisYCount +
    innerAxisXCount * innerAxisYCount + 18
  if (nodeCountUpperBound > 250_000)
    throw new Error("PostProcessingSolver: routingGrid exceeds the 250000-node composite-grid invariant limit")
