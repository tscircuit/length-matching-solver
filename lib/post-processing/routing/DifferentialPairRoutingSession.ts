...
export class DifferentialPairRoutingSession {
  ...
  step(): void {
    ...
    if (!prepared)
      throw new Error(`PostProcessingSolver: differential pair ${this.pairName} has no prepared state`)
    if (!this.search) {
  ...
  private prepare(): PreparedPair | null {
    const retained = (message: string): null => {
      this.result = {
        status: "retained",
        error: new Error(`PostProcessingSolver: differential pair ${this.pairName} ${message}`),
      }
      return null
    }
    const firstMatches = this.input.traces
      .map((trace, index) => ({ trace, index }))
      .filter(({ trace }) => trace.connection_name === this.input.pair.connectionNames[0])
    ...
    const normalSecondCost =
      Math.hypot(firstStart.x - second.points[0]!.x, firstStart.y - second.points[0]!.y) +
      Math.hypot(firstEnd.x - second.points.at(-1)!.x, firstEnd.y - second.points.at(-1)!.y)
    const reversedSecondCost =
      Math.hypot(firstStart.x - second.points.at(-1)!.x, firstStart.y - second.points.at(-1)!.y) +
      Math.hypot(firstEnd.x - second.points[0]!.x, firstEnd.y - second.points[0]!.y)
    const reverseSecond = reversedSecondCost + 1e-8 < normalSecondCost
    const secondStart = reverseSecond ? second.points.at(-1)! : second.points[0]!
    ...
    const attempts = EDGE_GAP_SAMPLES.flatMap((edgeGap) => {
      ...
      return ([preferredSide, preferredSide === 1 ? -1 : 1] as const).map((side) => {
  ...
  private complete(): void {
    ...
    this.candidates.sort((left, right) => this.score(left) - this.score(right) || left.edgeGap - right.edgeGap)
