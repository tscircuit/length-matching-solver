...
export const getGraphicsLayerForObstacle = (obstacle: Obstacle, layerCount: number): string => {
  const layers = obstacle.layers.map((layerName) => {
    ...
    if (!Number.isInteger(innerLayerIndex)) throw new Error(`LengthMatchingSolver: unknown layer name "${layerName}"`)
