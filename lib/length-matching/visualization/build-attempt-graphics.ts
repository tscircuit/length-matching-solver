...
export const buildAttemptGraphics = (input: {
  ...
  input.graphics.lines.push({
    ...
    label: `tested segment\n${input.attempt.toothCount} teeth`,
  ...
  if (!input.attempt.valid)
    input.graphics.lines.push({
      ...
      label: [
        `candidate rejected`,
        `depth ${input.attempt.predictedDepth.toFixed(3)}`,
        `error ${input.attempt.resultingError.toFixed(5)}`,
      ].join("\n"),
      layer: getGraphicsLayerForRoute(start.z),
    })
  input.graphics.points.push(
    {
      ...
      label: `${input.attempt.connectionName}\nsegment start`,
      layer: getGraphicsLayerForRoute(start.z),
    },
    {
      x: end.x,
