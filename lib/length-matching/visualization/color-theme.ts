...
export type LengthMatchingColorTheme = {
  getConnectionColor: (connectionName: string, routeConnectionName?: string) => string
...
const getDeterministicConnectionColor = (connectionName: string): string => {
  ...
  const characterSum = connectionName.split("").reduce((total, character) => total + character.charCodeAt(0), 0)
...
export const createLengthMatchingColorTheme = (colorMap: Record<string, string>): LengthMatchingColorTheme => ({
  getConnectionColor: (connectionName, routeConnectionName) => colorMap[connectionName] ?? (routeConnectionName ? colorMap[routeConnectionName] : undefined) ?? getDeterministicConnectionColor(connectionName),
